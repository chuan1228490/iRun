<template>
  <view class="page">
    <uni-nav-bar title="账户登录" backgroundColor="#FAFAF8" :border="false" statusBar fixed leftIcon="left" @clickLeft="onBack" color="#1C1B1A" />
    ...
    <scroll-view class="main-scroll" :style="{ height: scrollHeight + 'px' }" scroll-y enhanced :show-scrollbar="false">
      ...
      <view class="brand-section">
        ...
        <text class="brand-subtitle animate-fade-up delay-3">账户密码 / 验证码登录</text>
      </view>
...
const { countdown: regCountdown, startCooldown: startRegCooldown } = useSmsCooldown()
...
.main-scroll { box-sizing: border-box; width: 100%; padding: 0 48rpx; padding-bottom: 60rpx; }

.brand-section { display: flex; flex-direction: column; align-items: center; padding-top: 64rpx; padding-bottom: 48rpx; }
.brand-logo { width: 140rpx; height: 140rpx; border-radius: 50%; margin-bottom: 20rpx; box-shadow: var(--shadow-md); }
.brand-title { font-size: 44rpx; font-weight: 700; color: var(--text-primary); letter-spacing: 2rpx; }
.brand-subtitle { font-size: 24rpx; color: var(--text-secondary); margin-top: 6rpx; }
